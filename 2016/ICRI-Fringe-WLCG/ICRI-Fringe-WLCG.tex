...
\begin{frame}
  atlas processing direct std batch jobs.
  alice processing, job agents and leave it to central services, vobox.
  ops jobs fail due to starvation from alice.
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item the return of 28 grid nodes, on infiniband.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item ALICE concurrent jobs from 700 to 1600.
...
\begin{itemize}
  ...
  \item HLT replica for me and another.
  \item O2 dev for O2 development with dds/mesos/pbs.
  \item Local Storage for users, eos and lustre.
  \item 50/50 sl6/centos7 in prep for docker migration.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item r10k requires a mapping of git branches.\\
...
\begin{frame}
  puppet explanation, with rspec etc.
  hiera.

was planning to build on work i did at tlabs but that was destroyed :( 
